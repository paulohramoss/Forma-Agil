export type PaymentStatus = 'Pago' | 'Atrasado' | 'Em dia';
...
export interface ParticipantInput {
  ...
  value: number;
  whatsapp?: string;
  history?: Array<Omit<PaymentHistoryEntry, 'id'>>;
}
...
const pollStore: Poll = {
  ...
  options: [
    ...
    { id: cryptoId(), label: 'Grupo Samba e Som', votes: 17 },
  ],
  totalVotes: 77,
};
...
let communicationQueue: CommunicationMessage[] = [
  {
    id: cryptoId(),
    channel: 'WhatsApp',
    title: 'Cobranca parcela marco',
    body: 'Ola! Lembrando que a parcela de marco vence dia 15. Qualquer duvida estamos por aqui.',
  ...
  },
  {
    id: cryptoId(),
    channel: 'Email',
    title: 'Enquete: atracoes para a festa',
    body: 'Participem da enquete para escolher a banda da festa! Link disponivel no aplicativo.',
    audience: 'Todos os formandos',
...
export async function addParticipant(input: ParticipantInput): Promise<Participant> {
  await delay();
  const history = input.history ? input.history.map((entry) => ({ ...entry, id: cryptoId() })) : [];
...
export async function importParticipants(list: ParticipantInput[]): Promise<Participant[]> {
  ...
  const imported = list.map((item) => {
    const history = item.history
      ? item.history.map((entry) => ({ ...entry, id: cryptoId() }))
      : [];
    return {
      id: cryptoId(),
      name: item.name,
